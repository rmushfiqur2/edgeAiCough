...
def get_uncertainty_per_image(model, input_image, T=15, normalized=False):
    ...
    if normalized:
        ...
        p_hat = F.softmax(net_out, dim=1) # (T, num_classes)
    p_hat = p_hat.detach().cpu().numpy()  # (T, num_classes)
    p_bar = np.mean(p_hat, axis=0) # (num_classes)

    temp = p_hat - np.expand_dims(p_bar, 0) # (T, num_classes)
    epistemic = np.dot(temp.T, temp) / T # (num_classes, num_classes)
    epistemic = np.diag(epistemic) # (num_classes)

    # I don't understand the equation of aleatoric. however, it is something mean(y1) - mean((y1)^2), max when y1=0.5
    aleatoric = np.diag(p_bar) - (np.dot(p_hat.T, p_hat) / T) # (num_classes, num_classes)
